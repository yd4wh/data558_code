...
def ovo_classifier(X_train, y_train, X_val, y_val, X_test):
    """
    ...
    for f in range(X_test.shape[0]):
        data = pd.DataFrame(prediction_test[:,f].astype(int))
        # Use mode to find the most frequent class
        assignment = data.mode()
        # In the case of a tie, use random int generator for the indexing of a random mode
        if assignment.shape[0] > 1:
            ...
            assign = assignment.loc[0,0]
        y_test_pred.append(assign)
        
    return np.array(beta_matrix), np.array(obj_matrix), np.array(y_val_pred), np.array(y_test_pred), misclassification_error
...
if __name__ == '__main__':
    ...
    beta, obj, y_val, y_test, error = ovo_classifier(X_train,y_train,X_val,y_val,X_test)
